...
def ppo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    approx_ent = tf.reduce_mean(-logp) # here logp is just logp_old, using old policy to calulate
    ...
    def update():
        ...
        for i in range(train_pi_iters):
            kl, _ = sess.run([approx_kl, train_pi], feed_dict=inputs)
            kl = mpi_avg(kl)
            if kl > 1.5 * target_kl:
                logger.log('Early stopping policy training at step %d due to max kl'%i)
                break
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            a, v_t, logpt = sess.run(get_action_ops, feed_dict={x_ph: o.reshape(1, -1)})

            o2, r, d, _ = env.step(a[0])
            ep_ret += r
            ep_len += 1

            buf.store(o, a, r, v_t, logpt)
...
if __name__ == '__main__':
    import argparse
    # import os
    # os.environ['CUDA_VISIABLE_DIVICES'] = '3'
    ...
    args = parser.parse_args('--cpu 1 --steps 1000 --epochs 1000'.split())
