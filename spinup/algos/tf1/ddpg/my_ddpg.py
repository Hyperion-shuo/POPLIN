...
def ddpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
    ...
    # assum all dimensions share same bound
    act_limit = env.action_space.high[0]

    # share information about policy dim and bound
    ...
    print('\nNumbers of parameters: \t pi %d, \t q: %d, \t total: %d\n'%var_counts)

    back_up = tf.stop_gradient(r_ph + gamma*(1-d_ph)*q_pi_targ)

    pi_loss = -tf.reduce_mean(q_pi) 
    q_loss = tf.reduce_mean((q-back_up)**2)
    ...
    start_time, train_batch_time = time.time(), 0
    ...
    for t in range(total_steps):
        ...
        # ignore done signal if it comes from hitting the time
        # horizon
        ...
        # update o to most recent observation
        # critical and easy to ignore
        o = o2

        # End of traj, reset
        if d or ep_len==max_ep_len:
            logger.store(EpLen=ep_len, EpRet=ep_ret)
            o, ep_len, ep_ret = env.reset(), 0, 0
        ...
        train_batch_time += time.time() - train_start_time
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            logger.log_tabular('TrainBatchTime', train_batch_time)
            logger.dump_tabular()
            train_batch_time = 0
...
if __name__ == '__main__':
    ...
    args = parser.parse_args('--env MBRLHalfCheetah-v0 --exp_name mbrlhalfcheetah'.split())
